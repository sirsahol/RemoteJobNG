            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black text-text-main uppercase tracking-[0.2em]">
                  Verified Integrity Score: <span className="text-emerald-600 dark:text-emerald-400">{Math.min(40 + (badges.length * 12), 100)}%</span>
                </span>
              </div>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {stats.map((stat, i) => (
            <div 
              key={stat.label} 
              className={`glass-card p-8 border-glass-border bg-gradient-to-br ${stat.color} animate-in fade-in slide-in-from-bottom-4 duration-700`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <section className="glass-card p-8 border-glass-border relative overflow-hidden">
              <div className="absolute top-0 right-0 p-8 opacity-[0.03] dark:opacity-5">
                <svg className="w-32 h-32 text-text-main" fill="currentColor" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
              </div>
              ...
              {applications.length === 0 ? (
                <div className="py-12 text-center">
                  <p className="text-text-muted font-medium mb-4">No active transmissions detected.</p>
                  <Link href="/jobs" className="text-blue-600 dark:text-blue-400 font-bold hover:underline">Establish first connection</Link>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {applications.map(app => (
                    <div key={app.id} className="group p-5 rounded-2xl bg-glass-surface border border-glass-border hover:border-blue-500/30 transition-all flex items-center justify-between">
                      <div className="flex gap-4 items-center">
                        <div className="w-12 h-12 rounded-xl bg-glass-surface border border-glass-border flex items-center justify-center font-bold text-text-main shadow-lg overflow-hidden">
                          {app.job?.company_name?.[0] || app.company_name?.[0] || "J"}
                        </div>
                        <div>
                          <h3 className="font-bold text-text-main group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{app.job?.title || "Job Position"}</h3>
                          <p className="text-text-muted text-[10px] font-black uppercase tracking-widest mt-1">{app.job?.company_name || app.company_name || "Confidential"}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className={`text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${
                          app.status === "accepted" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" :
                          app.status === "rejected" ? "border-red-500/30 bg-red-500/10 text-red-600 dark:text-red-400" :
                          "border-blue-500/30 bg-blue-500/10 text-blue-600 dark:text-blue-400"
                        }`}>
                          {app.status || "Pending"}
                        </span>
                        <p className="text-text-muted/40 text-[9px] mt-2 font-black uppercase tracking-tighter">Syncing... 2d</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="glass-card p-8 border-glass-border relative overflow-hidden">
              ...
              <div className="space-y-4">
                {neuralMatches.length === 0 ? (
                  <div className="py-12 text-center bg-glass-surface rounded-3xl border border-dashed border-glass-border group hover:border-indigo-500/30 transition-colors">
                    <p className="text-text-muted text-xs font-bold uppercase tracking-widest mb-4">Neural Profile Incomplete</p>
                    <Link href="/profile/edit" className="text-indigo-600 dark:text-indigo-400 font-bold text-sm hover:text-text-main transition-colors">Update Bio to Sync</Link>
                  </div>
                ) : (
                  neuralMatches.map(job => (
                    <Link href={`/jobs/${job.slug}`} key={job.id} className="flex items-center justify-between p-5 rounded-2xl bg-glass-surface border border-glass-border hover:border-indigo-500/40 transition-all group relative overflow-hidden">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold border border-indigo-500/10">
                          {job.company_name?.[0]}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-text-main group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{job.title}</p>
                          <p className="text-[10px] text-text-muted uppercase font-black tracking-wider mt-0.5">{job.company_name}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-black text-indigo-600 dark:text-indigo-400">94.8%</p>
                        <p className="text-[8px] text-text-muted font-bold tracking-tighter">Correlation</p>
                      </div>
                    </Link>
                  ))
                )}
            ...
            {/* Saved Jobs Island */}
            <section className="glass-card p-8 border-glass-border">
              <h2 className="text-xl font-bold text-text-main mb-6 flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {savedJobs.length === 0 ? (
                  <p className="text-text-muted text-sm py-4 col-span-full">Your watchlist is empty.</p>
                ) : (
                  savedJobs.map(saved => (
                    <Link key={saved.id} href={`/jobs/${saved.job?.slug || saved.job?.id}`} className="p-5 rounded-2xl bg-glass-surface border border-glass-border hover:bg-glass-surface/50 transition-all block group">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-bold text-text-main group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors text-sm">{saved.job?.title || "Saved Job"}</h4>
                        <span className="text-blue-600 dark:text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity">→</span>
                      </div>
                      <p className="text-text-muted text-[10px] font-black uppercase tracking-widest">{saved.job?.company_name || ""}</p>
                    </Link>
                  ))
                )}
              </div>
          ...
          <div className="space-y-8">
            {/* Real-Time Job Pulse */}
            <section className="glass-card p-8 border-glass-border overflow-hidden relative">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-text-main">Network Pulse</h2>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-red-500 rounded-full animate-ping"></div>
                  <span className="text-[10px] font-black text-text-muted uppercase tracking-widest">LIVE</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex gap-4 items-start animate-in slide-in-from-right duration-500" style={{ animationDelay: `${i * 200}ms` }}>
                    <div className="w-8 h-8 rounded-lg bg-glass-surface flex items-center justify-center text-sm shrink-0 border border-glass-border">
                      {pulse.icon}
                    </div>
                    <div>
                      <p className="text-[11px] text-text-main/70 font-bold leading-snug">{pulse.text}</p>
                      <p className="text-[9px] text-text-muted font-black uppercase mt-1">{pulse.time}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-glass-border">
                ...
                <div className="h-1 bg-glass-surface rounded-full mt-2 overflow-hidden">
            ...
            <section className="glass-card p-8 border-glass-border">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-text-main">Alert Config</h2>
                <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
              </div>

              {showAlertForm && (
                <form onSubmit={handleCreateAlert} className="space-y-4 mb-6 p-4 rounded-xl bg-glass-surface border border-blue-500/20 animate-in zoom-in-95 duration-300">
                  <input
                    type="text"
                    placeholder="PROTOCOL NAME"
                    value={newAlert.name}
                    onChange={e => setNewAlert({...newAlert, name: e.target.value})}
                    className="w-full bg-bg-page border border-glass-border rounded-xl p-3 text-[10px] font-black uppercase tracking-widest text-text-main focus:outline-none focus:border-blue-500 transition-all"
                  />
                  <input
                    type="text"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <select
                      ...
                      className="bg-bg-page border border-glass-border rounded-xl p-3 text-[10px] font-black uppercase tracking-widest text-text-main focus:outline-none"
                    >
              ...
              <div className="space-y-3">
                {alerts.map(alert => (
                  <div key={alert.id} className="p-4 rounded-xl bg-glass-surface border border-glass-border hover:border-text-muted/20 transition-all">
                    <div className="flex items-center justify-between mb-2">
                       <h3 className="font-bold text-text-main text-xs tracking-tight">{alert.name || alert.keywords}</h3>
                       <button onClick={() => handleDeleteAlert(alert.id)} className="text-text-muted hover:text-red-500 transition-colors">
            ...
            <section className="glass-card p-8 border-glass-border bg-gradient-to-br from-indigo-600/10 to-blue-600/10 dark:from-indigo-600/20 dark:to-blue-600/20 group cursor-pointer hover:from-indigo-600/20 hover:to-blue-600/20 dark:hover:from-indigo-600/30 dark:hover:to-blue-600/30 transition-all">
               <div className="flex items-center gap-3 mb-3">
                 ...
                 <h3 className="text-text-main font-bold">Neural Resume Sync</h3>
               </div>
               <p className="text-text-muted text-[11px] mb-4 font-medium leading-relaxed">Our 2026 intelligence engine will automatically map your dossier to the global grid. Phase 5 initialization pending.</p>
               <div className="w-full bg-glass-surface border border-glass-border py-3 rounded-xl text-text-muted font-black text-[9px] uppercase tracking-[0.2em] text-center">
