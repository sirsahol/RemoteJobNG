    ...
    <div className={`fixed top-0 left-0 w-full z-50 transition-all duration-700 px-4 md:px-8 ${scrolled ? 'py-4' : 'py-8'}`}>
      <nav className={`max-w-7xl mx-auto glass-card px-6 py-3 flex items-center justify-between transition-all duration-500 ${scrolled ? 'shadow-2xl' : ''}`}>
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          {isAuthenticated ? (
            <>
              <div className="flex items-center gap-4 md:gap-6 pr-4 md:pr-6 border-r border-glass-border">
                  <Link
                    href={dashboardHref}
                    className={`text-[10px] font-black tracking-[0.2em] transition-all ${
                      pathname.startsWith("/dashboard") ? "text-blue-500" : "text-text-muted hover:text-text-main"
                    }`}
                  >
                    COMMAND
                  </Link>
                  ...
                  {isAdmin && (
                    <Link
                      ...
                      className={`text-[10px] font-black tracking-[0.2em] transition-all ${
                        pathname.startsWith("/admin") ? "text-indigo-500" : "text-text-muted hover:text-text-main"
                      }`}
                    >
                      CENTRAL
                    </Link>
                  ...
                  <div className="relative group">
                    <Link href="/notifications" className="text-text-muted hover:text-text-main transition-all block">
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                      </svg>
                      {unreadCount > 0 && (
                        <span className="absolute -top-1.5 -right-1.5 bg-blue-500 text-white text-[9px] font-black rounded-full h-4 w-4 flex items-center justify-center border-2 border-bg-page shadow-lg shadow-blue-500/40">
              ...
              <button
                onClick={logout}
                className="text-text-muted hover:text-red-500 text-[10px] font-black tracking-[0.2em] transition-all"
              >
                EXIT
              </button>
            </>
            ...
            <div className="flex items-center gap-2 md:gap-4">
              <Link
                href="/login"
                className="text-text-muted hover:text-text-main px-4 py-2 text-[10px] font-black tracking-[0.2em] transition-all"
              >
                ACCESS
              </Link>
              <Link
